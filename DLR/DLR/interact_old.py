...
class ClusterEnvironment:
    ...
    def get_vmstat_all_nodes(self):
       ...
       # io_wait_array = np.array(io_wait_list)
        ...
        #io_wait_array = np.array(io_wait_list)
        mem_usage_array = np.array(mem_usage_list,dtype=np.float32)
        cpu_usage_array = np.array(cpu_usage_list,dtype=np.float32)
        io_wait_array = np.array(io_wait_list,dtype=np.float32)
        obs = np.array([affinity_array, mem_usage_array, cpu_usage_array, io_wait_array]).T
        #print (obs)
        return obs
