...
class ClusterEnvironment:
    ...
    def check_get_rows_and_get_obs(self, rows, job_id, url, fetch=False):
        ...
        try:
            ...
            while len(rows) != 3 and retries < 500:
                retries += 1
                #print('NO COSBENCH DATA FOUND FOR: {} retrying {} ..'.format(job_id, retries))
                rows = self.parse_html_for_cosbench(url)
                time.sleep(1)
            else:  # if in read_write_del stage
                if retries < 500:
                    #print('COSBENCH DATA FOUND FOR: {} after {} attempts..'.format(job_id, retries))
